...
public extension Reactive where Base: UIView {
    ...
    public var movedToSuperview: AnyPublisher<Void, Error> {

        methodInvoked(#selector(UIView.didMoveToSuperview))
            .map {[weak base] _ in base?.superview != nil }
            .prepend(base.superview != nil)
            .filter { $0 }
            .map { _ in }
            .prefix(1)
            .eraseToAnyPublisher()
...
public extension ReactiveCompatible where Self: NSObject {
    ...
    func kvo<Value: Equatable>(_ property: ReferenceWritableKeyPath<Self, Value>, rx: ValueSubject<Value>) -> Self {
        rx.removeDuplicates()
            .sink {
                self[keyPath: property] = $0
            }.store(in: self.cb.asBag)
