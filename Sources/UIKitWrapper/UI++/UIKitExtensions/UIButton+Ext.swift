...
extension UIButton {
    ...
    public var backgroundColors: UIControl.States<UIColor> {
        UIControl.States(get: { [weak self] _ in self?.backgroundColor }, set: { [weak self] in self?.setBackground(color: $0 ?? .clear, for: $1) })
    }
